...
@php
    ...
    $heads = [
        ['label' => '#', 'title' => 'ID'],
        ['label' => 'Profile List ID', 'title' => 'Profile List ID'],
                ...
                <div class="card-body">
                    ...
                    <x-adminlte-datatable id="table" :heads="$heads" head-theme="light" theme="light" striped hoverable
                        ...
                        @foreach($getProfilesDataFromListTasks as $task)
                            <tr>
                                <td>{{ $task->id }}</td>
                                <td>{{ $task->profile_list_id }}</td>
                                <td>{{ $task->personal_profile_to_work }}</td>
                                <td>{{ $task->status }}</td>
                    ...
                    {{ $getProfilesDataFromListTasks->links('vendor.pagination.bootstrap-5') }}
                </div>
            </div>
        </div>
    </div>
