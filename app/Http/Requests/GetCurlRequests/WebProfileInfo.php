...
class WebProfileInfo
{
     ...
     */
    public function getWebProfileInfo(Profile $profile): bool|\stdClass
    {
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://www.instagram.com/api/v1/users/web_profile_info/?username=$profile->username",
        ...
        curl_close($curl);
        $this->saveWebProfileInfo($response);
        return json_decode($response, false, 512, JSON_THROW_ON_ERROR);
    }

    private function saveWebProfileInfo($response): void
    {
        try {
            ...
            PersonalProfileData::updateOrCreate(
                ...
                [
                    ...
                    'edge_felix_combined_post_uploads' => $profile->edge_felix_combined_post_uploads,
                    'edge_felix_combined_draft_uploads' => $profile->edge_felix_combined_draft_uploads,
                    'edge_felix_video_timeline' => $profile->edge_felix_video_timeline,
                    'edge_felix_drafts' => $profile->edge_felix_drafts,
                    'edge_felix_pending_post_uploads' => $profile->edge_felix_pending_post_uploads,
                    'edge_felix_pending_draft_uploads' => $profile->edge_felix_pending_draft_uploads,
                    'edge_owner_to_timeline_media' => $profile->edge_owner_to_timeline_media,
                    'edge_saved_media' => $profile->edge_saved_media,
                    'edge_media_collections' => $profile->edge_media_collections
                ]);
        } catch (Exception $e) {
