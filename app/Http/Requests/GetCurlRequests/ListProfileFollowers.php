...
class ListProfileFollowers
{
    ...
    public function getProfileFollowers(int $profileIdToGetFollowers, GetFollowersTask $task, ?Profile $personalProfile): void
    {
        ...
        while ($maxId || $continue) {
            ...
            Log::info("Pause active (list) - " . $rand_pause);
            usleep($rand_pause);
            $i++;
        }
    }
    ...
    private function saveFollowers($users, $profileid): void //move to repository
    {
        foreach ($users as $user) {
            ...
            $handledProfileId = ProfileInfo::where('inst_id', $profileid)->first();
            $profileFollowers->profileInfo()->syncWithoutDetaching($handledProfileId?->id);
